...
(function() {
  ...
  const CONFIG = {
    title: '📢 网站公告',
    content: '欢迎访问本站！这是一条重要公告信息。<br>已优化最小化动画方向，现在会向左上角收缩。',
  ...
  function init() {
    ...
    btnMinimize.addEventListener('click', (e) => {
      ...
      // 1. 获取当前实际尺寸 (包含展开状态的内容)
      const currentW = container.offsetWidth;
      const currentH = container.offsetHeight;
      
      // 2. 目标尺寸 (对应 .minimized CSS 中的尺寸)
      const targetW = 140;
      const targetH = 48;
      
      // 3. 计算为了保持左上角不动，需要增加的右/下边距
      // 原理：宽度减小多少，右边距就增加多少，把元素往左"顶"住
