...
(() => {
  ...
  (() => {
    ...
    const buildPatchedAutocomplete = (originalAutocomplete, runSearch) => {
      ...
      const wrappedAutocomplete = function autocompletePagefind(selector, options, dataset) {
        ...
        const patchedDataset = {
          ...
          templates: {
            ...dataset.templates || {},
            footer: () => '<div class="search-footer">Search by <a href="https://pagefind.app/" rel="noopener noreferrer" target="_blank">Pagefind</a></div>'
          }
        };
    ...
    const initPatch = () => {
      ...
      if (typeof optionsForPagefind.baseUrl === "string" && optionsForPagefind.baseUrl.length > 0 && optionsForPagefind.baseUrl === bundlePath) {
        console.warn(
          "[cmpt-pagefind-component] pagefind.baseUrl equals bundlePath. This may produce incorrect result URLs (e.g. /pagefind/posts/...). Leave baseUrl empty unless your site is deployed under a subpath."
        );
      }
      const state = {
        loading: null,
      ...
      const runSearch = async (query) => {
        ...
        const rawMaxResultLength = Number(
          pagefindConfig.maxResultLength ?? pagefindConfig.maxresultlength ?? componentConfig.maxResultLength ?? searchConfig.maxResultLength ?? 10
        );
        ...
        if (mode === "advanced") {
          ...
          return records2.map(mapResultItem);
        }
