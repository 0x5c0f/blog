  ...
  class SimpleAnnouncementBar {
    ...
    getTabStyles() {
      ...
      return {
        ...
        borderRadius: isLeftSide ? '0 16px 16px 0' : '16px 0 0 16px',
        boxShadow: isLeftSide ?
          '3px 3px 16px rgba(0, 0, 0, 0.12), 0 0 0 1px rgba(255, 255, 255, 0.2), inset 0 0 0 1px rgba(255, 255, 255, 0.3)' :
          '-3px 3px 16px rgba(0, 0, 0, 0.12), 0 0 0 1px rgba(255, 255, 255, 0.2), inset 0 0 0 1px rgba(255, 255, 255, 0.3)',
        position: 'fixed',
        zIndex: '999',
        display: 'flex',
    ...
    getTabPositionStyles(savedPosition) {
      if (savedPosition) {
        // 判断公告栏最后位置是在左侧还是右侧
        const announcementWidth = 320; // 公告栏宽度
        const windowWidth = window.innerWidth;
        const savedLeft = savedPosition.left ? parseInt(savedPosition.left) : null;
        const savedRight = savedPosition.right ? parseInt(savedPosition.right) : null;
        
        // 如果有明确的left值，说明在左侧
        if (savedLeft !== null && savedPosition.left !== 'auto') {
          return {
        ...
        // 如果有明确的right值，或者left是auto，说明在右侧
        if (savedRight !== null && savedPosition.right !== 'auto' || savedPosition.left === 'auto') {
          return {
            top: savedPosition.top || '20px',
            right: '0px',
            bottom: 'auto',
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .simple-announcement-tab.left-tab {
        ...
        box-shadow: 3px 3px 16px rgba(0, 0, 0, 0.12), 0 0 0 1px rgba(255, 255, 255, 0.2), inset 0 0 0 1px rgba(255, 255, 255, 0.3) !important;
      }
