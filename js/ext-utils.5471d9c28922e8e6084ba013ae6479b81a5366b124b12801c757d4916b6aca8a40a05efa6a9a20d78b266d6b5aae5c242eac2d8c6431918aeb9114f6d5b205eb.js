...
// 添加一个方法来判断是否为移动设备
const createDeviceChecker = () => {
  const isMobileDevice = () => {
    return window.matchMedia('only screen and (max-width: 680px)').matches;
  };

  return {
    check: isMobileDevice // 公开一个方法来访问私有函数
  };
};

const deviceChecker = createDeviceChecker();
console.log(deviceChecker.check()); // 可以调用
// console.log(isMobileDevice()); // 这里会报错，因为 isMobileDevice 是私有的
