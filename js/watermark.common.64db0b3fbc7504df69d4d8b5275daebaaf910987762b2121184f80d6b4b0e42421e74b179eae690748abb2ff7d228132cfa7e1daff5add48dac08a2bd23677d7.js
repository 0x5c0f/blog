...
(function() {
    // 默认配置
    var defaultConfig = {
        startTime: '2022-06-21',                // 网站开始运行的时间
        textWatermark: {
            watermarkCount: 3,                  // 默认文本水印数量
        ...
        mobileConfig: {                         // 手机端配置
            type: 'text',                       // 手机端使用印章水印
            textWatermark: {
                watermarkCount: 1                // 手机端文本水印数量
            },
            sealWatermark: {
                watermarkCount: 1                // 手机端印章水印数量
            }
        },
        pcConfig: {                             // PC端配置
            type: 'seal',                       // PC端使用文本水印
            textWatermark: {
                watermarkCount: 3                // PC端文本水印数量
            },
            sealWatermark: {
                watermarkCount: 2                // PC端印章水印数量
            }
        }
    };
    ...
    function initWatermark(userConfig) {
        ...
        // 检查设备并应用对应配置
        var deviceConfig = ExtUtils.isMobile() ? config.mobileConfig : config.pcConfig;
        var watermarkType = deviceConfig.type;

        if (watermarkType === 'text') {
            config.textWatermark.watermarkCount = deviceConfig.textWatermark.watermarkCount;
            createTextWatermark(config.textWatermark);
        } else if (watermarkType === 'seal') {
            config.sealWatermark.watermarkCount = deviceConfig.sealWatermark.watermarkCount;
