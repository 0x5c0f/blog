...
(function() {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* 最小化状态 */
      .announcement-container.minimized {
      ...
      .announcement-container.minimized .announcement-header,
      .announcement-container.minimized .announcement-content,
      .announcement-container.minimized .announcement-footer {
        display: none;
      }

      .announcement-container.minimized .announcement-toggle {
        display: flex !important;
      }

      /* 最小化动画 - 从右下向左上收缩 */
      @keyframes shrinkToTopLeft {
        0% {
          width: ${CONFIG.style.width};
          height: auto;
          transform: scale(1);
      ...
      .announcement-container.animating-minimize {
        animation: shrinkToTopLeft ${CONFIG.animation.duration} ${CONFIG.animation.easing} forwards;
        transform-origin: top left;
      }

      /* 展开动画 - 从左上向右下展开 */
      @keyframes expandToBottomRight {
        0% {
          width: 140px;
          height: 48px;
          transform: scale(1);
      ...
      .announcement-container.animating-expand {
        animation: expandToBottomRight ${CONFIG.animation.duration} ${CONFIG.animation.easing} forwards;
        transform-origin: top left;
      }
  ...
  function createAnnouncementHTML() {
    return `
      <div class="announcement-toggle" title="点击展开">
        📢 网站公告
      </div>
  ...
  function enableDrag(container) {
    ...
    if (savedPosition) {
      ...
      setTranslate(savedPosition.x, savedPosition.y, container);
    }

    const header = container.querySelector('.announcement-header');
    const toggleBtn = container.querySelector('.announcement-toggle');
    ...
    function setTranslate(xPos, yPos, el) {
      ...
      // 保留其他transform属性，只更新translate
      if (currentTransform && currentTransform.includes('scale')) {
        el.style.transform = `translate(${xPos}px, ${yPos}px) ${currentTransform.match(/scale\([^)]+\)/)?.[0] || ''}`;
      } else {
  ...
  function initAnnouncement() {
    ...
    // 启用拖动
    const dragController = enableDrag(container);
    ...
    minimizeBtn.addEventListener('click', () => {
      container.classList.add('animating-minimize');
      
      setTimeout(() => {
        container.classList.remove('animating-minimize');
    ...
    toggleBtn.addEventListener('mouseup', (e) => {
      ...
      if (clickDuration < 300 && moveDistance < 10) {
        ...
        container.classList.add('animating-expand');
        
        setTimeout(() => {
          container.classList.remove('animating-expand');
        }, 400);
      }
    });
